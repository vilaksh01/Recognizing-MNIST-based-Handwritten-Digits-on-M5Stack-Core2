...
#endif
#if (NEUTON_MODEL_HAS_CLASSES_RATIO == 1)
			+ sizeof(modelClassesRatio)
#endif
#if (NEUTON_MODEL_HAS_NEGPOS_RATIO == 1)
			+ sizeof(modelNegPosRatio)
#endif
			;
}
...
#if (NEUTON_MODEL_HAS_CLASSES_RATIO == 1)
static float probability_curve(float probability, float ratio)
{
	double coeff = 1;
	coeff = ((1.0 / (1 + exp((probability - 0.5) * 10))) / (1.0 / (1.0 - ratio))) + ratio;
	return probability * (ratio / coeff);
}
#endif
...
#endif
#if (NEUTON_MODEL_HEADER_VERSION > 1) && (NEUTON_MODEL_TASK_TYPE < 2) && (NEUTON_MODEL_HAS_CLASSES_RATIO == 1)
		modelOutput[i] = probability_curve(modelOutput[i], modelClassesRatio[i]);
#endif
	}
